...
interface StoreContextType {
  ...
  refetchShiftReports: (forceRefresh?: boolean) => Promise<void>;
  exportAndResetShiftReports: () => Promise<{ totalShifts: number; totalSales: number }>;
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    if (supabase) {
      const fetchInitialData = async () => {
        ...
        try {
          ...
          if (!shiftErr && Array.isArray(remoteShifts)) {
  ...
  useEffect(() => {
    ...
    const refetchShiftReports = async () => {
      const { data, error } = await cachedSupabaseQuery<ShiftReport[]>(
        'shift_reports',
        async () => {
          const res = await client
            .from('shift_reports')
        ...
        { table: 'shift_reports', forceRefresh: true }
      );
      if (!cancelled && !error && Array.isArray(data)) {
  ...
  const refetchShiftReports = useCallback(async (forceRefresh = true) => {
    if (!supabase) return;
    try {
      const { data, error } = await cachedSupabaseQuery<ShiftReport[]>(
        'shift_reports',
        ...
        { table: 'shift_reports', forceRefresh }
      );
      if (!error && Array.isArray(data)) {
        setShiftReports(data);
      }
    } catch (err) {
      console.warn('[StoreContext] Error refetching shift_reports:', err);
    }
  }, []);
  ...
  // 8b. Owner: Export & Reset Shift Reports (.xlsx & Permanent Database Deletion)
  const exportAndResetShiftReports = async (): Promise<{
    totalShifts: number;
    totalSales: number;
  }> => {
    let allShifts: ShiftReport[] = [];
    let targetShiftIds: string[] = [];

    // 1. Query ALL rows from Supabase shift_reports
    if (supabase) {
      try {
        const { data: dbShifts, error: shiftErr } = await supabase
          .from('shift_reports')
          .select('*')
          .order('created_at', { ascending: false });

        if (!shiftErr && Array.isArray(dbShifts)) {
          allShifts = dbShifts as ShiftReport[];
          targetShiftIds = allShifts.map((s) => s.id);
        } else if (shiftErr) {
          console.warn('[StoreContext] Failed fetching shift_reports from Supabase:', shiftErr);
        }
      } catch (err) {
        console.warn('[StoreContext] Exception fetching shift_reports for export:', err);
      }
    }

    // Fallback if offline or empty remote but local exists
    if (allShifts.length === 0 && shiftReports.length > 0) {
      allShifts = [...shiftReports];
      targetShiftIds = allShifts.map((s) => s.id);
    }

    const totalShifts = allShifts.length;
    const totalSales = allShifts.reduce((sum, s) => sum + (Number(s.total_sales) || 0), 0);

    if (totalShifts === 0) {
      showToast(
        language === 'ar' ? 'لا توجد تقارير ورديات لتصديرها' : 'No shift reports available to export',
        'info'
      );
      return { totalShifts: 0, totalSales: 0 };
    }

    // 2. Generate Excel (.xlsx) file using SheetJS
    const wb = XLSX.utils.book_new();

    const shiftsSheetData = allShifts.map((sr) => ({
      'Shift #': sr.shift_number,
      'Cashier Name': sr.cashier_name,
      'Start Time': sr.start_time ? new Date(sr.start_time).toLocaleString() : '',
      'End Time': sr.end_time ? new Date(sr.end_time).toLocaleString() : '',
      'Orders Count': sr.total_orders_count ?? 0,
      'Total Sales (EGP)': Number(sr.total_sales) || 0,
      'Cash Sales (EGP)': Number(sr.cash_sales) || 0,
      'Digital Sales (EGP)': Number(sr.digital_sales) || 0,
      'Expenses Total (EGP)': Number(sr.expenses_total) || 0,
      'System Expected Cash (EGP)': Number(sr.system_expected_cash) || 0,
      'Cashier Reported Cash (EGP)': Number(sr.cashier_reported_cash) || 0,
      'Discrepancy (EGP)': Number(sr.discrepancy) || 0,
      'Notes': sr.notes || '',
      'Created At': sr.created_at ? new Date(sr.created_at).toLocaleString() : '',
    }));

    const shiftsWs = XLSX.utils.json_to_sheet(shiftsSheetData);
    XLSX.utils.book_append_sheet(wb, shiftsWs, 'Shift Reports');

    // 3. Trigger download of .xlsx file
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([wbout], {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    });
    const dateStr = new Date().toISOString().slice(0, 10);
    const fileName = `Chocolate_House_Shift_Reports_Export_${dateStr}.xlsx`;
    ...
    // 4. ONLY AFTER download is triggered: PERMANENTLY DELETE shift_reports from Supabase
    if (supabase) {
      try {
        console.info(`[StoreContext] 🗑️ Permanently deleting ${targetShiftIds.length} shift reports from Supabase...`);
        if (targetShiftIds.length > 0) {
          const { error: delErr } = await supabase
            .from('shift_reports')
            .delete()
            .in('id', targetShiftIds);

          if (delErr) {
            console.error('[StoreContext] Supabase delete shift_reports error:', delErr);
            throw delErr;
          }
        } else {
          ...
          if (delAllErr) {
            console.error('[StoreContext] Supabase delete all shift_reports error:', delAllErr);
          }
        }
        console.info('[StoreContext] ✅ Shift reports permanently deleted from Supabase.');
      } catch (err: any) {
        console.error('[StoreContext] Failed deleting shift reports from Supabase:', err);
        showToast(
          language === 'ar'
            ? `تنبيه: تم تنزيل الملف، ولكن تعذر حذف تقارير الورديات من السحابة: ${err.message || err}`
            : `Warning: Excel file downloaded, but could not delete shift reports from cloud: ${err.message || err}`,
          'warning'
        );
      }
    }

    // 5. Update local state & invalidate cache so table becomes empty in real time
    setShiftReports([]);
    ...
    showToast(
      language === 'ar'
        ? `تم تنزيل ملف الإكسيل وحذف ${totalShifts} تقرير وردية بنجاح من قاعدة البيانات`
        : `Excel file downloaded and ${totalShifts} shift reports permanently deleted from database`,
      'success'
    );

    return { totalShifts, totalSales };
  };
